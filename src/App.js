...
function App() {
  ...
  const changeElementOrder = (array, id, moveTo = "up") => {
    ...
    const toIndex =
      moveTo === "up" ? fromIndex - 1 : moveTo === "down" ? fromIndex + 1 : 1;
  ...
  const changeElementOrderInNestedList = (array, id, parentId, moveTo = "up") =>
    array.map((item) => {
      ...
      return {
        ...item,
        ...(item.subList && {
          subList: changeElementOrderInNestedList(
            item.subList,
            id,
            parentId,
            moveTo
  ...
  const addElementToSubList = (array, clickedId, newItem) =>
    array.map((item) => {
      if (item.id === clickedId) {
        return {
          ...item,
          subList: [...item.subList, newItem]
        };
      }
      return {
        ...item,
        ...(item.subList && {
          subList: addElementToSubList(item.subList, clickedId, newItem)
        })
      };
    });
  ...
  const deleteElementFromList = (array, id, parentId) =>
    array.map((item) => {
      if (item.id === parentId) {
        const updatedSubList = item.subList.filter(
      ...
      return {
        ...item,
        ...(item.subList && {
          subList: deleteElementFromList(item.subList, id, parentId)
        })
      };
    });
  ...
  const handleChangeOrder = (id, parentId, moveTo) => {
    ...
    if (!parentId) updatedList = changeElementOrder(list, id, moveTo);
      ...
      updatedList = changeElementOrderInNestedList(list, id, parentId, moveTo);
    setList(updatedList);
  };
    ...
    <Container>
      <ul>
        {list.map((listItem, index) =>
          getListItem(listItem, index === list.length - 1, index === 0)
        )}
      </ul>
      <div>
        <input
        ...
        />
        <Button onClick={() => onAdd(null, null, name, true)} type="primary">
          Add
        </Button>
      </div>
    </Container>
